...
if __name__=='__main__':
	...
	while line:
		...
		if node4.get_tcp_packet() != 0 and (float(end_time) - float(start_time)) != 0:
			...
			if log.link_dst == '0':
				...
				for l in queue02.logs:
					if l.pck_id == log.pck_id:
						...
						loss_time_ave_list.append(sum_loss_time / (node4.r_tcp + 1) * 1000)
	...
	through_put = (node4.get_tcp_packet() * 8) / (float(end_time) - float(start_time))
	print(' - - - - - - - - - - - - - - - - - ')
	print('start_time = ' + str(start_time))
	print('end_time = ' + str(end_time))
	print('time = ' + str(float(end_time) - float(start_time)))
	print(' - - - - - - - - - - - - - - - - - ')
	print('loss / get (num) = ' + str(drop04packet) + ' / ' + str(queue02.tcp_en))
	print('loss / get (%)   = ' + str(100 * round(float(drop04packet)/float(queue02.tcp_en),5)) + ' %')
	print(' - - - - - - - - - - - - - - - - - ')
	print('total loss time(sec)  = ' + str(sum_loss_time))
	print('node4 receive pck num = ' + str(node4.r_tcp))
	print('ave loss time(sec)  = ' + str(sum_loss_time / node4.r_tcp))
	print('ave loss time(msec) = ' + str((sum_loss_time / node4.r_tcp)*1000))
	print(' - - - - - - - - - - - - - - - - - ')
	print('jitter(sec)  = ' + str(jitter))
	print('jitter(msec) = ' + str(jitter * 1000))
	print(' - - - - - - - - - - - - - - - - - ')
	print('get_packet(bit)  = ' + str(node4.get_tcp_packet() * 8) + ' bit')
	print('get_packet(byte) = ' + str(node4.get_tcp_packet()) + ' byte')
	print(' - - - - - - - - - - - - - - - - - ')

	print('through_put = node4.get_packet (byte) * 8 / (end_time - start_time)')
	print('through_put = ' + str(node4.get_tcp_packet()) + ' * 8 / (' + str(end_time) + ' - ' + str(start_time) + ')')
	print('through_put = ' + str(node4.get_tcp_packet() * 8) + ' / ' + str(float(end_time) - float(start_time)))
	...
	axR.plot(time_list_for_drop, loss_time_ave_list, label='ave loss time')
	axR.plot(time_list_for_drop, loss_time_list, label='loss time')
	axR.plot(time_list_for_drop, jitter_list, label='jitter')
